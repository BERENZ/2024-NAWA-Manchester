...
for (r in 1:n_sims) {
  ...
  error_age <- as.matrix(prop.table(xtabs(~ age_m + age, data = S_A_err_valid_sample), margin = 2))
  error_eth <- as.matrix(prop.table(xtabs(~ ethnic_m + ethnic, data = S_A_err_valid_sample), margin = 2))
  S_A_err[, age := NULL]
  S_A_err[, ethnic := NULL]
  S_A_err[, age := age_m]
...
```{r}
colMeans(results_df[,10:15]) |> kable()
```
