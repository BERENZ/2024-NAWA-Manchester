...
This is the first sumulation study for the project. In this simulation study I check the performance of MC-SIMEX method for mis-classification where at least one auxiliary variable is measured with error. Then I verify the case when the target variable is also measured with error. 
...
For this simulation study I am intereted in $\mu_Y$ which is population mean (share) of $Y$. In the simulation study I compare the following estimators:
...
X1 <- sample(c("S", "M", "L"), N, prob = c(0.7, 0.2, 0.1), replace = T) ## X1 variable
X1 <- factor(X1, c("S", "M", "L"))
X2 <- rbinom(N, 1, prob = 0.7) ## X2 variable
X2 <- factor(X2, 0:1, c("Pub", "Priv")) 
X3 <- rbinom(N, 1, prob = 0.5) ## X3 variable 
Y <- rbinom(N, 1, plogis(-1 + 0.5*(X1 == "L") + 0.5*(X2=="Priv") - X3)) ## target variable
pi_A <- plogis(-4 + 3*(X1 == "L")) ## selection to non-probability sample
pop_data <- data.frame(X1,X2,X3,Y,pi_A) ## population data frame
head(pop_data)
```

The known population mean of $Y$ is `r round(mean(Y)*100,2)`%.

Now, I set up the confusion matrices
...
for (b in 1:n_reps) {
  ...
  results[[b]] <- data.frame(b, 
                             ...
                             mu_mc_mi = weighted.mean(predict(res_mi, sample_b_svy$variables, type = "response"), 
